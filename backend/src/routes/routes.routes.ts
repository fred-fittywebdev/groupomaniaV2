...
import { DislikePost, LikePost } from './../controllers/post.controller';
...
import { Users, CreateUser, GetUser, UpdateUser, DeleteUser, ExportUser } from './../controllers/user.controller';
...
export const routes = (router: Router) => {
    ...
    router.post('/api/users', AuthMiddleware,PermissionMiddleware('roles'), CreateUser)
    ...
    router.post('/api/export', AuthMiddleware,PermissionMiddleware('roles'), ExportUser)

    // Post routes
    router.get('/api/posts', AuthMiddleware, PermissionMiddleware('posts'), Posts)
    router.post('/api/users/:id/post',AuthMiddleware,PermissionMiddleware('users'), CreatePostUser)
    router.post('/api/users/:id/comment', AuthMiddleware,PermissionMiddleware('users'), CreateUSerComment)
    router.post('/api/posts', AuthMiddleware,PermissionMiddleware('users'), CreatePost)
    router.get('/api/post/:id', AuthMiddleware,PermissionMiddleware('posts'), GetPost)
    router.put('/api/post/:id', AuthMiddleware,PermissionMiddleware('posts'), UpdatePost)
    router.delete('/api/post/:id', AuthMiddleware,PermissionMiddleware('posts'), DeletePost)
    // On like un post
    router.put('/api/post/:id/like', AuthMiddleware,PermissionMiddleware('users'), LikePost)
    // On dislike un post
    router.put('/api/post/:id/dislike', AuthMiddleware,PermissionMiddleware('users'), DislikePost)
    // Comment routes
    router.get('/api/comments', AuthMiddleware,PermissionMiddleware('users'), getComments)
    router.post('/api/posts/:id/comment',AuthMiddleware,PermissionMiddleware('users'), CreateCommentPost)
    router.get('/api/comments/:id', AuthMiddleware,PermissionMiddleware('posts'), GetComment)
    router.put('/api/comments/:id', AuthMiddleware,PermissionMiddleware('users'), UpdateComment)
    router.delete('/api/comments/:id', AuthMiddleware,PermissionMiddleware('posts'), DeleteComment)
